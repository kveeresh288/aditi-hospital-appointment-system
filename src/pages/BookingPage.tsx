...
import { getDoctorById, type BookingDoctor } from '../data/bookingData';
import type { Appointment, BookingDraft, PatientIdentity } from '../types';

const PATIENT_STORAGE_KEY = 'aditi_patient_identity';

function loadStoredIdentity(): PatientIdentity {
  if (typeof window === 'undefined') return { fullName: '', phoneNumber: '' };
  try {
    const stored = sessionStorage.getItem(PATIENT_STORAGE_KEY);
    return stored ? JSON.parse(stored) : { fullName: '', phoneNumber: '' };
  } catch {
...
export function BookingPage() {
  ...
  const selectedDoctor: BookingDoctor | undefined = getDoctorById(draft.doctorId);

  const handleIdentityContinue = (identity: PatientIdentity) => {
    sessionStorage.setItem(PATIENT_STORAGE_KEY, JSON.stringify(identity));
    setDraft((prev) => ({ ...prev, patientName: identity.fullName, patientPhone: identity.phoneNumber }));
    setStep(1);
  };

  const handleDoctorContinue = (doctor: BookingDoctor) => {
        ...
        <div className="max-w-3xl mx-auto">
          ...
          <div className="bg-white sm:bg-transparent">
            ...
            {step === 5 && confirmedAppointment && (
              <ConfirmationStep appointment={confirmedAppointment} onBookAnother={handleBookAnother} />
            )}
          </div>
        </div>
      </main>
